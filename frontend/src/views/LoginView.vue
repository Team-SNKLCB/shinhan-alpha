<template>
    <div>
        ...
        <div style="margin-top: 10px; background-color: #e9f0ff; height: 625px">
            <div class="title">
                <p>아이디 로그인</p>
                <p style="font-size: 12px; color: #666464">아이디와 비밀번호를 입력해주세요.</p>
            </div>
            <div class="login-form">
                <input id="id" v-model="id" placeholder="아이디" />
                <input id="password" type="password" v-model="password" placeholder="비밀번호" />
                <input id="confirm_password" type="password" v-model="confirm_password" placeholder="비밀번호 확인" /><br />
                <div style="display: flex; justify-content: right; align-items: center; margin-top: 10px; margin-right: 15px">
                    ...
                    <img @click="changeToggle" v-else-if="save_toggle === true" src="../assets/on-btn.png" style="width: 80px; height: 40px" />
                </div>
            </div>
            <div @click="login" style="background-color: #4a8ce2; color: white" class="make-account-btn">로그인</div>
        </div>
    </div>
</template>
